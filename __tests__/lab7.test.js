describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    */
    ...
    for (let i = 0; i < prodItemsData.length; i++) {
      const current = prodItemsData[i];
      if (!current.title || current.title.length === 0) { allArePopulated = false; }
      if (!current.price || current.price.length === 0) { allArePopulated = false; }
      if (!current.image || current.image.length === 0) { allArePopulated = false; }
    }
    expect(allArePopulated).toBe(true);
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodItemHandle = await page.$('product-item');
    const shadowRootHandle = await prodItemHandle.getProperty('shadowRoot');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allButtonsRemove = await page.$$eval('product-item', items => {
      return items.every(itm => itm.shadowRoot.querySelector('button').innerText === 'Remove from Cart');
    });
    const cartCount = await page.$eval('#cart-count', el => el.innerText);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.reload({ waitUntil: 'networkidle0' });
    const allButtonsAdd = await page.$$eval('product-item', items => {
      return items.every(itm => itm.shadowRoot.querySelector('button').innerText === 'Add to Cart');
    });
    const cartCount = await page.$eval('#cart-count', el => el.innerText);
